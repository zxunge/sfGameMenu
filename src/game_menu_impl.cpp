...
void Menu::writeText(std::string str, sf::Font* font, unsigned int size, float x, float y, const Color color)
{
    ...
    text.setOrigin({textRect.size.x / 2.0f, 0});
    if (x - textRect.size.x / 2.0f < 0)
    {
        x = textRect.size.x / 2 + _style.PaddingTitle.left;
    }
    if (x + textRect.size.x / 2.0f > _window.getSize().x)
    {
        x = _window.getSize().x - textRect.size.x / 2 + _style.PaddingTitle.left;
    }
